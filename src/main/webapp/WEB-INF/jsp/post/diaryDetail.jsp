			...
			<div class="ml-3">
				<c:choose>
					<c:when test="${postList.like }">
						<i class="bi bi-heart-fill unlike-icon " style="font-size:5rem; color: red; cursor: pointer;" data-post-id="${postList.id }"></i>
					</c:when>
					<c:otherwise>
						<i class="bi bi-heart like-icon" style="font-size:5rem; cursor: pointer;" data-post-id="${postList.id }"></i>
					</c:otherwise>
				</c:choose>
			</div>
			<h4>좋아요 ${postList.likeCount }개</h4>
			</div>
		</div>	
		<div class="mt-5 justify-content-center">
			<c:forEach var="comment" items="${postList.commentList }">
				<hr>
				<div class="d-flex">							
			...
			<div class="d-flex align-items-center justify-content-center contain">
				<input type="text" class="col-10 form-control" id="commentInput${postList.id }"></input>
				<button type="button" class="btn btn-primary comment-btn" data-post-id="${postList.id }" data-category="1">게시</button>
			</div>
		</div>
		</section>
